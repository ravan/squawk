...
- **Palette** — the floating, draggable toolbar (tools, colors, stroke controls, text size, actions).
...
- **Annotation.** One immutable record in the session model: `rect`, `ruler`, `ellipse`, `arrow`, `pen`, `text`, `label`, or `color-sample`.
- **Ruler.** A measured rectangle Annotation with width and height labels in document CSS pixels.
- **Color sample.** The Eyedropper Annotation that stores one clicked document point, its sampled uppercase hex color, and the Stroke width and Stroke style used by its marker.
...
- **Armed tool.** The tool that currently owns pointer input on the Overlay. Exactly one of interact/select/rect/ruler/ellipse/arrow/pen/text/picker/eyedropper/eraser.
- **Eyedropper.** The tool that captures the visible tab after a click, reads the clicked viewport pixel, and commits one Color sample at the corresponding document point.
